...
public class ShowAllSurvey extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (userInfoOnSession != null) {
			...
			// 「survey」テーブルのデータを全件抽出し、投稿した時間が早い順にソート
			List<SurveyDto> list  = new ArrayList<SurveyDto>();
			ShowAllSurveyBL logic = new ShowAllSurveyBL();
			if (request.getParameter("sort") != null && request.getParameter("sort").equals("satisfaction")) {
				list = logic.executeSelectSurveySortedBySatisfactionLevel();  // 満足度でソートするメソッドを呼び出す
			} else {
				list = logic.executeSelectSurvey();  // デフォルトは時間でソート
			}
